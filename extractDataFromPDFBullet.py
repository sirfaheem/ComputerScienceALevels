...
for page_num in range(pdf_reader.getNumPages()):
    ...
    with open('temp.txt', 'wb') as temp_file:
        extract_text_to_fp(page, temp_file, output_type='text', laparams=laparams, output_dir=None)
        text = temp_file.getvalue().decode().replace('\n', ' ')
    ...
    question_number = ''
    question_number_match = re.search(r'^(\d+(\s+\([a-z]\))*)', text[question_start:question_end])
